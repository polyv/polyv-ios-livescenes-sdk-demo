...
@implementation PLVSAImageMessageCell
...
- (void)layoutSubviews {
    ...
    if ([self.model isProhibitMsg]) {
        ...
        // nickLabel跟图片之间距离 8
        originY += 8;
        CGSize imageViewSize = [PLVSAImageMessageCell calculateImageViewSizeWithMessage:self.model.message];
...
- (void)loadImageWithModel:(PLVChatModel *)model {
    ...
    NSURL *imageURL = [PLVSAImageMessageCell imageURLWithMessage:message];
...
+ (CGFloat)cellHeightWithModel:(PLVChatModel *)model loginUserId:(NSString *)loginUserId cellWidth:(CGFloat)cellWidth{
    ...
    if ([model isProhibitMsg]) {
        ...
        CGSize imageViewSize = [PLVSAImageMessageCell calculateImageViewSizeWithMessage:model.message];
...
- (void)resendButtonAction {
    ...
    if (self.resendImageHandler &&
        self.msgState == PLVImageMessageSendStateFailed) {
