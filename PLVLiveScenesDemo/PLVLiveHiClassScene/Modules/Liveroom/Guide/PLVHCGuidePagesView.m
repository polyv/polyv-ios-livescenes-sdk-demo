...
- (void)layoutSubviews {
    ...
    if (_guidePagesType == PLVHCGuidePagesType_Device) {
        ...
        CGFloat classImageViewCentY  = linkMicHeight + 8 + CGRectGetHeight(self.classImageView.bounds)/2;
        if ([PLVHCUtils sharedUtils].isPad) {
            linkMicHeight = linkNumber > 6 ?  115.3 : 82;
            classImageViewCentY = linkMicHeight + (self.bounds.size.height - linkMicHeight) / 2 - 36 * 2 - 14 * 2;
        }
        self.classImageView.center = CGPointMake(CGRectGetWidth(self.bounds) - edgeInsetsRight - CGRectGetWidth(self.classImageView.bounds)/2, classImageViewCentY);
