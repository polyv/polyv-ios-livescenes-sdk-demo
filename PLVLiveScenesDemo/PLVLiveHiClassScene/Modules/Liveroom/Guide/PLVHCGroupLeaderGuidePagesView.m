...
- (void)layoutSubviews {
    ...
    CGFloat guideImageViewCenterY = linkMicHeight + 8 + CGRectGetHeight(self.guideImageView.bounds)/2;
    if ([PLVHCUtils sharedUtils].isPad) {
        linkMicHeight = linkNumber > 6 ?  115.3 : 82;
        guideImageViewCenterY = linkMicHeight + (self.bounds.size.height - linkMicHeight) / 2 - 36 * 2 - 14 * 2;
    }
    self.guideImageView.center = CGPointMake(CGRectGetWidth(self.bounds) - edgeInsetsRight - CGRectGetWidth(self.guideImageView.bounds)/2, guideImageViewCenterY);
