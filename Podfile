...
target 'PolyvLiveScenesDemo' do
  ...
  # 保利威 多场景 SDK
  pod 'PLVLiveScenesSDK', '1.26.0'

  # 保利威 手机开播场景 需依赖的库
  pod 'PLVBytedEffectSDK', '4.4.2'
  pod 'PLVBeautyEffect','1.0.0'

  pod 'PLVBusinessSDK', '1.26.0', :subspecs => ['Beauty']
  
  # 保利威 SM2加密 需依赖的库
  pod 'PLVLOpenSSL', '~> 1.1.12100'
  pod 'PLVFoundationSDK', '1.25.1', :subspecs => ['CryptoUtils']
...
target 'PLVScreenShareExtension' do
  use_frameworks!
  pod 'PLVBusinessSDK', '1.26.0', :subspecs => ['AbstractBSH','ReplayKitExt']
  pod 'PLVFoundationSDK', '1.25.1', :subspecs => ['AbstractBase']
end
