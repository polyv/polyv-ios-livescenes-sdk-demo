...
- (void)notifyListenerDidReceiveCheckVoiceWarnings:(NSArray<PLVCheckVoiceWarningModel *> *)warningModels {
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(chatroomViewModel:didReceiveCheckVoiceWarnings:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.delegate chatroomViewModel:self didReceiveCheckVoiceWarnings:warningModels];
        });
    }
}
