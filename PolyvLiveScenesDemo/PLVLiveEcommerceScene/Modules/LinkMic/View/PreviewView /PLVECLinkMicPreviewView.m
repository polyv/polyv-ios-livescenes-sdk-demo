...
@interface PLVECLinkMicPreviewView ()
...
@property (nonatomic, strong) PLVECLinkMicPreMediaSwitchView *cameraSwitchView;
@property (nonatomic, strong) PLVECLinkMicPreMediaSwitchView *micSwitchView;
...
@implementation PLVECLinkMicPreviewView
...
- (void)saveUserLinkMicPreConfig {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:dict forKey:kPLVECUserLinkMicPreConfig];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

- (void)synchLocalLinkMicPreConfig {
    NSDictionary *dict = [[NSUserDefaults standardUserDefaults] objectForKey:kPLVECUserLinkMicPreConfig];
...
- (void)createInviteLinkMicTimer {
    ...
    self.acceptInviteLinkMicLimitTs = self.currentTimestamp + kPLVECLinkMicInvitationAnswerTTL * 1000;
...
- (void)callbackForAcceptLinkMicInvitation {
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(plvECLinkMicPreviewViewAcceptLinkMicInvitation:)]) {
        [self.delegate plvECLinkMicPreviewViewAcceptLinkMicInvitation:self];
    }
}

- (void)callbackForCancelLinkMicInvitationReason:(PLVECCancelLinkMicInvitationReason)reason {
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(plvECLinkMicPreviewView:cancelLinkMicInvitationReason:)]) {
        [self.delegate plvECLinkMicPreviewView:self cancelLinkMicInvitationReason:reason];
    }
}

- (void)callbackForInviteLinkMicTTLCallback:(void (^)(NSInteger ttl))callback {
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(plvECLinkMicPreviewView:inviteLinkMicTTL:)]) {
        [self.delegate plvECLinkMicPreviewView:self inviteLinkMicTTL:callback];
    }
}
...
- (UIView *)contentView {
    if (!_contentView) {
        ...
        _contentView.backgroundColor = [PLVColorUtil colorFromHexString:@"#262523"];
...
- (UIImageView *)tipsImageView {
    if (!_tipsImageView) {
        ...
        _tipsImageView.image = [PLVECUtils imageForWatchResource:@"plvec_linkmic_pre_tips_icon"];
    }
...
- (UIButton *)rejectButton {
    if (!_rejectButton) {
        ...
        _rejectButton.layer.borderColor = [PLVColorUtil colorFromHexString:@"#FFA611"].CGColor;
...
- (UIButton *)linkMicButton {
    if (!_linkMicButton) {
        ...
        _linkMicButton.backgroundColor = [PLVColorUtil colorFromHexString:@"#FFA611"];
...
- (void)rejectLinkMicButtonAction {
    ...
    [self callbackForCancelLinkMicInvitationReason:PLVECCancelLinkMicInvitationReason_Manual];
}
...
- (void)linkMicButtonAction {
    ...
    [[PLVCaptureDeviceManager sharedManager] requestAuthorizationWithType:type grantedRefuseTips:message completion:^(BOOL granted) {
        if (granted) {
            ...
            [weakSelf callbackForCancelLinkMicInvitationReason:PLVECCancelLinkMicInvitationReason_Permissions];
        }
...
- (void)inviteLinkMicTimerAction {
    ...
    if (leftTime <= 0) {
        [self showLinkMicPreviewView:NO];
        [self callbackForCancelLinkMicInvitationReason:PLVECCancelLinkMicInvitationReason_Timeout];
        return;
    }
        
    if((kPLVECLinkMicInvitationAnswerTTL - leftTime)%9 == 0) { // 更新倒计时时间
        __weak typeof(self) weakSelf = self;
...
@implementation PLVECLinkMicPreMediaSwitchView
...
- (UISwitch *)mediaSwitch {
    if (!_mediaSwitch) {
        ...
        _mediaSwitch.onTintColor = [PLVColorUtil colorFromHexString:@"#FFA611"];
    }
    return _mediaSwitch;
}
