...
- (void)updateViewLayouForUserVideoSizeChanged:(NSString *)linkMicUserId {
    ...
    if (isLandscape) {
        self.collectionView.frame = CGRectMake(0, self.collectionBackgroundView.bounds.size.height * 0.166, self.collectionBackgroundView.bounds.size.width, self.collectionBackgroundView.bounds.size.width/videoSize.width * videoSize.height);
    } else {
        self.collectionView.frame = self.collectionBackgroundView.bounds;
    }
}
