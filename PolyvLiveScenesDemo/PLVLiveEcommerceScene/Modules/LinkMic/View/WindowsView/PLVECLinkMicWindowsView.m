...
@property (nonatomic, assign) NSInteger showSeparatelyUserIndex; // 单独显示的用户下标（大于等于0 才需要单独显示）
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _enableSeparateLinkMicLayout = NO;
        _showSeparatelyUserIndex = -1;
...
- (void)filterShowSeparatelyUserIndex {
    ...
    if (self.enableSeparateLinkMicLayout &&
    ...
    if (separatelyUserIndex > -1) {
        ...
        self.linkMicUserCount = 1; // 将显示数量设置为1，这样collectionView只显示主讲用户
