//
//  PLVECLinkMicControlBarProtocol.h
//  PLVLiveScenesDemo
//
...
#define PLVECLinkMicControlBar_ShiftTime 0.3  // 位移时长 (控制栏展开、收起、隐藏过程的位移时长)
#define PLVECLinkMicControlBar_CommonTime 0.3 // 通用变化时长 (电话图标旋转、文字渐隐动画变更的动画时长)

static const BOOL PLVECLinkMicControlBarMicDefaultOpen = YES;           // Bar 麦克风按钮 默认开关值
static const BOOL PLVECLinkMicControlBarCameraDefaultOpen = NO;         // Bar 摄像头按钮 默认开关值
static const BOOL PLVECLinkMicControlBarSwitchCameraDefaultFront = YES; // Bar 切换前后摄像头按钮 默认前置值

/// 枚举
/// 连麦控制栏 状态 (此状态与 ‘控制栏是否隐藏'状态 互为独立)
typedef NS_ENUM(NSUInteger, PLVECLinkMicControlBarStatus) {
    PLVECLinkMicControlBarStatus_Default = 0, // 默认状态，控制栏隐藏
    PLVECLinkMicControlBarStatus_Open    = 2, // 显示 ‘申请连麦’
    PLVECLinkMicControlBarStatus_Waiting = 4, // 显示 ‘请求中...’
    PLVECLinkMicControlBarStatus_Joined  = 6, // 已连麦，显示相关的控制按钮
};

/// 连麦控制栏 类型
typedef NS_ENUM(NSUInteger, PLVECLinkMicControlBarType) {
    PLVECLinkMicControlBarType_Video = 0, // 默认状态，视频连麦类型
    PLVECLinkMicControlBarType_Audio = 1, // 音频连麦类型
};


#pragma mark - [ Abstract Protocol ]
@protocol PLVECLinkMicControlBarDelegate;

/// PLVECLinkMicControlBar 连麦控制栏 对象抽象协议
///
/// @note 注意与 PLVECLinkMicControlBarDelegate 区分；
///       若业务所需，希望自定义一个自家的连麦控制栏，则可遵循 <PLVECLinkMicControlBarProtocol> 此抽象协议来实现，实现后则自动与其他模块对接起来。
@protocol PLVECLinkMicControlBarProtocol <NSObject>

@required
#pragma mark 可配置
/// Delegate
@property (nonatomic, weak) id <PLVECLinkMicControlBarDelegate> delegate;
/// 是否可移动 (默认为 YES)
...
@property (nonatomic, assign) PLVECLinkMicControlBarStatus status; // 当前连麦控制栏的状态
@property (nonatomic, assign) PLVECLinkMicControlBarType barType; // 连麦控制栏类型
...
/// 同步其他状态栏的状态
///
/// @param controlBar 其他遵循 <PLVECLinkMicControlBarProtocol> 抽象协议的状态栏对象
...
/// PLVECLinkMicControlBar 连麦控制栏 代理协议方法
///
/// @note 注意与 PLVECLinkMicControlBarProtocol 区分
@protocol PLVECLinkMicControlBarDelegate <NSObject>
...
onOffButtonClickedCurrentStatus:(PLVECLinkMicControlBarStatus)status;
