//
//  PLVECLinkMicPortraitControlBar.m
//  PLVLiveScenesDemo
//
...
@interface PLVECLinkMicPortraitControlBar () <CAAnimationDelegate>
...
/// view hierarchy
///
/// (PLVECLinkMicPortraitControlBar) self
...
@implementation PLVECLinkMicPortraitControlBar
...
- (void)controlBarStatusSwitchTo:(PLVECLinkMicControlBarStatus)status {
    ...
    if (status == PLVECLinkMicControlBarStatus_Default) { // 默认状态，控制栏隐藏
        ...
        NSString *textLabelString = self.barType == PLVECLinkMicControlBarType_Audio ? @"申请音频连麦": @"申请视频连麦";
    ...
    if (isPad) {
        ...
        if (isSplitView && self.cameraButtonEnable && self.status == PLVECLinkMicControlBarStatus_Joined && self.barType == PLVECLinkMicControlBarType_Video) {
            if (!self.cameraButton.selected) {
                [self cameraButtonAction:self.cameraButton];
                [PLVECUtils showHUDWithTitle:nil detail:@"分屏模式下无法使用摄像头，已自动关闭摄像头" view:self.superview afterDelay:3.0];
            } else {
                [PLVECUtils showHUDWithTitle:nil detail:@"分屏模式下无法使用摄像头，已自动禁用摄像头" view:self.superview afterDelay:3.0];
            }
...
- (void)onOffButtonAction:(UIButton *)button{
    if ([PLVLivePictureInPictureManager sharedInstance].pictureInPictureActive) {
        [PLVECUtils showHUDWithTitle:@"小窗播放中，不支持连麦" detail:@"" view:self.superview];
        return;
    }
