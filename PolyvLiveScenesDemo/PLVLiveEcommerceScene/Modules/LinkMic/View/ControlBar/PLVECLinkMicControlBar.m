...
@interface PLVECLinkMicControlBar () <CAAnimationDelegate>
...
@implementation PLVECLinkMicControlBar
...
- (void)layoutSubviews{
    ...
    if (self.barType == PLVECLinkMicControlBarType_Audio) {
        /// 音频连麦类型
...
- (void)changeBarType:(PLVECLinkMicControlBarType)barType {
    self.barType = barType;
    [self setNeedsLayout];
}

- (void)controlBarStatusSwitchTo:(PLVECLinkMicControlBarStatus)status {
    _status = status;
    ...
    if (status == PLVECLinkMicControlBarStatus_Default) { // 默认状态，控制栏隐藏
        ...
        [self resetButtons];
    }else if (status == PLVECLinkMicControlBarStatus_Open) { // 显示 ‘申请连麦’
        NSString *textLabelString = self.barType == PLVECLinkMicControlBarType_Audio ? PLVLocalizedString(@"申请音频连麦"): PLVLocalizedString(@"申请视频连麦");
        ...
        [self resetButtons];
    }else if (status == PLVECLinkMicControlBarStatus_Waiting){ // 显示 ‘请求中...’
    ...
    }else if (status == PLVECLinkMicControlBarStatus_Joined){ // 已连麦，显示相关的控制按钮
        [self unfoldSelfView];
...
- (void)refreshControlBarFrame{
    ...
    CGFloat x;
    if (self.status == PLVECLinkMicControlBarStatus_Default) {
...
- (void)synchControlBarState:(id<PLVECLinkMicControlBarProtocol>)controlBar{
...
- (void)showSelfViewWithAnimation:(BOOL)show{
    ...
    [UIView animateWithDuration:(show ? PLVECLinkMicControlBar_CommonTime : 0.1) animations:^{
...
- (void)updateLinkMicRequestIndex:(NSInteger)index {
    if (self.status != PLVECLinkMicControlBarStatus_Waiting) {
        return;
    }
...
- (void)setupData{
    self.canMove = YES;
    self.status = PLVECLinkMicControlBarStatus_Default;
    self.hiddenSelf = YES;
...
- (void)resetButtons{
    // 恢复默认值
    _cameraButton.selected = !PLVECLinkMicControlBarCameraDefaultOpen;
    _switchCameraButton.selected = !PLVECLinkMicControlBarCameraDefaultOpen;
    _switchCameraButtonFront = PLVECLinkMicControlBarSwitchCameraDefaultFront;
    _micButton.selected = !PLVECLinkMicControlBarMicDefaultOpen;
...
- (CGFloat)selfWidth {
    ...
    if (_foldSelf) { // 折叠时
        ...
        if(_status == PLVECLinkMicControlBarStatus_Open) { // 显示‘申请连麦’时
        ...
        } else if (self.status == PLVECLinkMicControlBarStatus_Waiting) { // 显示‘请求中’时
            CGSize textLabelSize = [self.textLabel sizeThatFits:CGSizeMake(MAXFLOAT, self.textLabel.bounds.size.height)];
            w = textLabelSize.width + 54.0;
        } else if(_status == PLVECLinkMicControlBarStatus_Joined) { // 已连麦时
            if (self.barType == PLVECLinkMicControlBarType_Audio) { // 音频连麦
                w = 122.0;
            } else { // 视频连麦
                w = 200.0;
            }
...
- (UIButton *)onOffButton{
    if (!_onOffButton) {
        ...
        [_onOffButton setImage:[self getImageWithName:@"plvec_linkmic_phone"] forState:UIControlStateNormal];
...
- (UIButton *)cameraButton{
    if (!_cameraButton) {
        ...
        [_cameraButton setImage:[self getImageWithName:@"plvec_linkmic_camera_open"] forState:UIControlStateNormal];
        [_cameraButton setImage:[self getImageWithName:@"plvec_linkmic_camera_close"] forState:UIControlStateSelected];
...
- (UIButton *)micButton{
    if (!_micButton) {
        ...
        _micButton.alpha = 0;
        [_micButton setImage:[self getImageWithName:@"plvec_linkmic_mic_open"] forState:UIControlStateNormal];
        [_micButton setImage:[self getImageWithName:@"plvec_linkmic_mic_close"] forState:UIControlStateSelected];
...
- (UIButton *)hideButton{
    if (!_hideButton) {
        ...
        _hideButton.alpha = 0;
        [_hideButton setImage:[self getImageWithName:@"plvec_linkmic_control_hide"] forState:UIControlStateNormal];
...
- (void)unfoldSelfView{
    ...
    if (self.status == PLVECLinkMicControlBarStatus_Joined) {
        [self onOffButtonIconChange:NO];
        [self onOffButtonRotate:YES];
        [self onOffButtonColorChange:PLVColor_OnOffButton_Red];
        [self mediaControlButtonsShow:YES];
    ...
    __weak typeof(self) weakSelf = self;
    [UIView animateWithDuration:PLVECLinkMicControlBar_ShiftTime delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
...
- (void)onOffButtonIconChange:(BOOL)showSettingIcon{
    if (showSettingIcon) {
        [self.onOffButton setImage:[self getImageWithName:@"plvec_linkmic_setting"] forState:UIControlStateNormal]; /// 展示‘齿轮’图标
    }else{
        [self.onOffButton setImage:[self getImageWithName:@"plvec_linkmic_phone"] forState:UIControlStateNormal]; /// 展示‘电话’图标
    }
}
...
- (void)onOffButtonColorChange:(UIColor *)color{
    __weak typeof(self) weakSelf = self;
    [UIView animateWithDuration:PLVECLinkMicControlBar_CommonTime animations:^{
...
- (void)onOffButtonRotate:(BOOL)rotate{
    if (_phoneRotated != rotate) {
        ...
        animation.duration = PLVECLinkMicControlBar_CommonTime;
        if (rotate) {
...
- (void)textLabelContentChange:(NSString *)text{
    ...
    __weak typeof(self) weakSelf = self;
    NSTimeInterval totalTime = PLVECLinkMicControlBar_CommonTime;
    [UIView animateWithDuration:(totalTime / 2.0) animations:^{
        weakSelf.textLabel.alpha = 0;
    } completion:^(BOOL finished) {
        if (weakSelf.status != PLVECLinkMicControlBarStatus_Joined) {
            [UIView animateWithDuration:(totalTime / 2.0) animations:^{
...
- (void)textLabelShow:(BOOL)show{
    __weak typeof(self) weakSelf = self;
    [UIView animateWithDuration:PLVECLinkMicControlBar_ShiftTime animations:^{
...
- (void)shouldShowCameraEnableAlert {
    ...
    if (isPad) {
        ...
        if (isSplitView && self.cameraButtonEnable && self.status == PLVECLinkMicControlBarStatus_Joined && self.barType == PLVECLinkMicControlBarType_Video) {
            if (!self.cameraButton.selected) {
                [self cameraButtonAction:self.cameraButton];
                [PLVECUtils showHUDWithTitle:nil detail:PLVLocalizedString(@"分屏模式下无法使用摄像头，已自动关闭摄像头") view:self.superview afterDelay:3.0];
            } else {
                [PLVECUtils showHUDWithTitle:nil detail:PLVLocalizedString(@"分屏模式下无法使用摄像头，已自动禁用摄像头") view:self.superview afterDelay:3.0];
            }
            self.cameraButtonEnable = NO;
            self.cameraButton.userInteractionEnabled = NO;
        } else if (!isSplitView || self.status != PLVECLinkMicControlBarStatus_Joined) {
            self.cameraButtonEnable = YES;
            self.cameraButton.userInteractionEnabled = YES;
            self.cameraButton.alpha = (self.status == PLVECLinkMicControlBarStatus_Joined && self.barType == PLVECLinkMicControlBarType_Video && !_foldSelf) ? 1.0 : 0.0;
        }
    }
}
...
- (void)tapGestureAction:(UITapGestureRecognizer *)tapGR {
    if (self.status == PLVECLinkMicControlBarStatus_Open || self.status == PLVECLinkMicControlBarStatus_Waiting) {
...
- (void)onOffButtonAction:(UIButton *)button{
    if ([PLVLivePictureInPictureManager sharedInstance].pictureInPictureActive) {
        [PLVECUtils showHUDWithTitle:PLVLocalizedString(@"小窗播放中，不支持连麦") detail:@"" view:self.superview];
        return;
    }
...
- (void)notifyListenerOnOffButtonClickedCurrentStatus {
    ...
    BOOL joinedAndFold = self.status == PLVECLinkMicControlBarStatus_Joined && self.foldSelf;
    if (self.status == PLVECLinkMicControlBarStatus_Open || joinedAndFold) {
        [self unfoldSelfView];
    }
    
    if (joinedAndFold == NO) {
        if ([self.delegate respondsToSelector:@selector(PLVECLinkMicControlBar:onOffButtonClickedCurrentStatus:)]) {
            [self.delegate PLVECLinkMicControlBar:self onOffButtonClickedCurrentStatus:self.status];
        }
    }
}
...
- (void)cameraButtonAction:(UIButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(PLVECLinkMicControlBar:cameraButtonClicked:openResult:)]) {
        __weak typeof(self) weakSelf = self;
        [self.delegate PLVECLinkMicControlBar:self cameraButtonClicked:wannaOpen openResult:^(BOOL openResult) {
...
- (void)switchCameraButtonAction:(UIButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(PLVECLinkMicControlBar:switchCameraButtonClicked:)]) {
        [self.delegate PLVECLinkMicControlBar:self switchCameraButtonClicked:_switchCameraButtonFront];
    }
}
...
- (void)micButtonAction:(UIButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(PLVECLinkMicControlBar:micCameraButtonClicked:)]) {
        [self.delegate PLVECLinkMicControlBar:self micCameraButtonClicked:!button.selected];
    }
}
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag{
    if (flag) {
        if (self.foldSelf && self.status == PLVECLinkMicControlBarStatus_Joined && !self.phoneRotated) {
