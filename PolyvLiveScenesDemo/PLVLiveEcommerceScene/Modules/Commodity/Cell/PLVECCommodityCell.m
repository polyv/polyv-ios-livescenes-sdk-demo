...
- (void)layoutSubviews {
    ...
    CGFloat positionY = CGRectGetMaxY(self.nameLabel.frame) + 4;
    ...
    self.selectButton.frame = CGRectMake(CGRectGetWidth(self.bounds) - CGRectGetWidth(self.selectButton.frame) , CGRectGetHeight(self.bounds)-28, CGRectGetWidth(self.selectButton.frame) , 28);
...
- (NSURL *)jumpLinkUrl {
    NSURL *jumpLinkUrl;
    // 跳转地址
    if (10 == self.model.linkType && [PLVFdUtil checkStringUseable:self.model.link]) { // 通用链接
        jumpLinkUrl = [NSURL URLWithString:self.model.link];
    } else if (11 == self.model.linkType && [PLVFdUtil checkStringUseable:self.model.mobileAppLink]) { // 多平台链接
...
- (void)setModel:(PLVCommodityModel *)model {
    ...
    if ([PLVFdUtil checkStringUseable:model.btnShow]) {
        [self.selectButton setTitle:model.btnShow forState:UIControlStateNormal];
        
        CGSize size = CGSizeMake(MAXFLOAT, 28);
        CGSize buttonSize = [model.btnShow boundingRectWithSize:size
                                                        options:NSStringDrawingTruncatesLastVisibleLine  | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading
                                                     attributes:@{ NSFontAttributeName:self.selectButton.titleLabel.font}
                                                        context:nil].size;
        self.selectButton.frame = CGRectMake(CGRectGetWidth(self.bounds) - buttonSize.width - 10, CGRectGetHeight(self.bounds) - 28, buttonSize.width + 10, 28);
    } else {
        self.selectButton.frame = CGRectMake(CGRectGetWidth(self.bounds) - 60, CGRectGetHeight(self.bounds) - 28, 60, 28);
    }
    self.selectButton.enabled = [self jumpLinkUrl] ? YES : NO;
    if (self.selectButton.enabled) {
        self.selectButton.backgroundColor = [UIColor colorWithRed:1 green:166/255.0 blue:17/255.0 alpha:1];
    } else {
        self.selectButton.backgroundColor = [UIColor colorWithRed:80/255.0 green:80/255.0 blue:80/255.0 alpha:1];
    }
    // 实际价格显示逻辑
...
- (UILabel *)productTagsLabel {
    ...
    tagsLabel.backgroundColor = [UIColor colorWithRed:1 green:24/255.0 blue:17/255.0 alpha:0.08];
    tagsLabel.textColor = [UIColor colorWithRed:1 green:143/255.0 blue:17/255.0 alpha:1];
