...
@interface PLVECLocalSystemMessageCell ()
...
@implementation PLVECLocalSystemMessageCell
...
- (void)layoutSubviews {
    ...
    CGFloat originX = 8.0;
    CGFloat maxWidth = self.cellWidth - originX * 2;

    CGFloat labelWidth = [PLVECLocalSystemMessageCell systemLabelWidth];
    ...
    self.bubbleView.frame = CGRectMake(0, 4.0, bubbleWidth, bubbleHeight);
...
- (void)updateWithModel:(PLVChatModel *)model cellWidth:(CGFloat)cellWidth {
    if (cellWidth <= 0 || ![PLVECLocalSystemMessageCell isModelValid:model]) {
        self.cellWidth = 0;
        return;
    }
...
+ (NSMutableAttributedString *)contentLabelAttributedStringWithMessage:(id)message {
    NSString *content = @"";
    if ([message isKindOfClass:[PLVLocalSystemMessage class]]) {
        content = ((PLVLocalSystemMessage *)message).content ?: @"";
    }
    return [[NSMutableAttributedString alloc] initWithString:content attributes:@{
...
+ (CGFloat)cellHeightWithModel:(PLVChatModel *)model cellWidth:(CGFloat)cellWidth {
    ...
    PLVLocalSystemMessage *message = (PLVLocalSystemMessage *)model.message;
    CGFloat maxWidth = cellWidth - 16.0;
    CGFloat labelWidth = [PLVECLocalSystemMessageCell systemLabelWidth];
    CGFloat contentMaxWidth = maxWidth - 8.0 * 2 - labelWidth - 4.0;
...
- (UILabel *)labelView {
    if (!_labelView) {
        ...
        _labelView.backgroundColor = [PLVColorUtil colorFromHexString:@"#454545"];
