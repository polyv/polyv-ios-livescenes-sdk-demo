...
static const NSInteger PLVECProductClickDisplayLimitPerTick = 5;
static const NSUInteger PLVECProductClickNicknameMaxLength = 5;
...
- (BOOL)shouldFilterEffectMessageModel:(PLVChatModel *)model {
    return (![self productMessageEffectEnabled] && [PLVECProductConversionEffectCell isModelValid:model]);
}
...
- (void)addPublicChatModel:(PLVChatModel *)model {
    ...
    if ([PLVECProductConversionEffectCell isModelValid:model]) {
        model.attributeString = [[NSMutableAttributedString alloc] initWithAttributedString:[PLVECProductConversionEffectCell conversionAttributedStringWithModel:model]];
        model.cellHeightForV = [PLVECProductConversionEffectCell cellHeightWithModel:model cellWidth:self.tableViewWidth];
        ...
        model.cellHeightForV = [PLVECLocalSystemMessageCell cellHeightWithModel:model cellWidth:self.tableViewWidth];
    }
    
    [self.publicChatArray addObject:model];
    if ([PLVECLocalSystemMessageCell isModelValid:model]) {
...
- (void)addPublicChatModels:(NSArray <PLVChatModel *> *)modelArray {
    ...
    for (PLVChatModel *model in modelArray) {
        ...
        if ([model isKindOfClass:[PLVChatModel class]]) {
            [self.publicChatArray addObject:model];
            if (model.user.specialIdentity || [PLVECLocalSystemMessageCell isModelValid:model]) {
                [self.partOfPublicChatArray addObject:model];
            }
        }
    }
...
- (void)notifyListenerSignInSuccessWithNickname:(NSString *)nickname {
...
- (void)notifyListenerProductClickEffectMessage:(NSString *)message {
...
- (void)productClickTimerAction {
    ...
    if (messageArray.count <= PLVECProductClickDisplayLimitPerTick) {
        [self addPublicChatModels:messageArray];
        return;
    }
    
    NSRange displayRange = NSMakeRange(0, PLVECProductClickDisplayLimitPerTick);
    NSArray *topMessages = [messageArray subarrayWithRange:displayRange];
    [self addPublicChatModels:topMessages];
    
    NSArray *overflowMessages = [messageArray subarrayWithRange:NSMakeRange(PLVECProductClickDisplayLimitPerTick, messageArray.count - PLVECProductClickDisplayLimitPerTick)];
...
- (void)productClickEventWithSubEvent:(NSString *)subEvent jsonDict:(NSDictionary *)jsonDict {
    ...
    [self notifyListenerProductClickEffectMessage:effectMessage];
...
- (NSString *)displayNicknameForProductClickMessage:(NSString *)nickName {
    ...
    NSString *prefix = [self productClickNicknamePrefixForString:nickName
                                                       maxLength:PLVECProductClickNicknameMaxLength
                                                    didTruncate:&truncated];
    if (truncated) {
...
- (void)socketMananger_didReceiveEvent:(NSString *)event
    ...
    if ([event isEqualToString:@"customMessage"] &&
        ...
        [self notifyListenerSignInSuccessWithNickname:nickname];
    }
}
...
- (void)socketMananger_didReceiveMessage:(NSString *)subEvent
    ...
    if ([subEvent isEqualToString:@"LOGIN"]) {   // someone logged in chatroom
        ...
        [self notifyListenerRewardSuccess:contentDict];
               ...
               [[subEvent uppercaseString] isEqualToString:@"PRODUCT_CLICK"] ||
               [[PLV_SafeStringForDictKey(jsonDict, @"EVENT") uppercaseString] isEqualToString:@"PRODUCT_CLICK"]) {
        [self productClickEventWithSubEvent:subEvent jsonDict:jsonDict];
    } else if ([subEvent isEqualToString:@"SIGN_IN_TIMES"] ||
               [PLV_SafeStringForDictKey(jsonDict, @"EVENT") isEqualToString:@"SIGN_IN_TIMES"]) {
        NSString *nickname = PLV_SafeStringForDictKey(jsonDict, @"nick");
