...
@implementation PLVECBottomSheet
...
- (void)resetWithAnimate {
    ...
    [UIView animateWithDuration:kBottomSheetAnimationDuration animations:^{
        if ([PLVECUtils sharedUtils].isLandscape &&
            self.superLandscape) {
            self.contentView.frame = CGRectMake(self.bounds.size.width - self.sheetLandscapeWidth - [PLVECUtils sharedUtils].areaInsets.right, 0, self.sheetLandscapeWidth + [PLVECUtils sharedUtils].areaInsets.right, self.bounds.size.height);
        } else {
