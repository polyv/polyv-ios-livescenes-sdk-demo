...
static const CGFloat kPLVECOnlineListCellHeight = 52.0;

@interface PLVECOnlineListSheet () <UITableViewDelegate,UITableViewDataSource>
...
@implementation PLVECOnlineListSheet
...
- (void)layoutSubviews {
    ...
    self.tableView.frame = CGRectMake(0, kPLVECOnlineListCellHeight, contentViewWidth, contentViewHeight - kPLVECOnlineListCellHeight - P_SafeAreaBottomEdgeInsets());
}
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[PLVECOnlineListSheetCell class] forCellReuseIdentifier:@"PLVECOnlineListSheetCell"];
...
- (void)refreshUserList {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(plvECOnlineListSheetNeedUpdateOnlineList:)]) {
        [self.delegate plvECOnlineListSheetNeedUpdateOnlineList:self];
    }
}
...
- (void)ruleButtonAction {
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(plvECOnlineListSheetWannaShowRule:)]){
        [self.delegate plvECOnlineListSheetWannaShowRule:self];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    PLVECOnlineListSheetCell *cell = [tableView dequeueReusableCellWithIdentifier:@"PLVECOnlineListSheetCell" forIndexPath:indexPath];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return kPLVECOnlineListCellHeight;
}
