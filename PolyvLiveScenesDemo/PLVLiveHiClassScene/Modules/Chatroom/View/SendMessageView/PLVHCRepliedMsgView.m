...
@implementation PLVHCRepliedMsgView
...
- (void)layoutUI {
    ...
    CGFloat originX = [PLVHCUtils sharedUtils].areaInsets.left + 63;
    
    CGFloat closeButtonOriginX = width - [PLVHCUtils sharedUtils].areaInsets.right - 40 - 52;
    ...
    CGFloat height = 0;
    if (self.imageMsg) {
        CGSize imageSize = [PLVHCRepliedMsgView calculateImageViewSizeWithMessage:self.imageMsg];
...
- (UIVisualEffectView *)effectView {
    if (!_effectView) {
        ...
        _effectView.backgroundColor = [PLVColorUtil colorFromHexString:@"#1E2236"];
    }
    return _effectView;
}
...
- (UIButton *)closeButton {
    if (!_closeButton) {
        UIImage *image = [PLVHCUtils imageForChatroomResource:@"plvhc_chatroom_btn_close"];
...
- (UILabel *)label {
    if (!_label) {
        ...
        _label.textColor = [PLVColorUtil colorFromHexString:@"#70727F"];
