...
- (void)showSettingViewWithLocalConfig:(NSDictionary *)configDict {
    //设置设备默认值
    if (![PLVFdUtil checkDictionaryUseable:configDict]) {
        return;
    }
    self.localPreviewUser = YES;
    self.micView.button.selected = !PLV_SafeBoolForDictKey(configDict, kSCMicrophoneEnableConfigKey);
    self.cameraView.button.selected = !PLV_SafeBoolForDictKey(configDict, kSCCameraEnableConfigKey);
    self.cameraSwitchView.button.selected = !PLV_SafeBoolForDictKey(configDict, kSCCameraIsFrontConfigKey);
    NSString *nickname = PLV_SafeStringForDictKey(configDict, kSCLocalPrevierUserNicknameKey);
    self.titleLabel.text = [NSString stringWithFormat:@"讲师-%@",nickname];
    NSString *imageURL = PLV_SafeStringForDictKey(configDict, kSCLocalPrevierUserAvatarURLKey);
    [self updateUserAvatarWithImageURL:imageURL];
...
- (void)updateSettingContentView {
    NSMutableArray <PLVHCLinkMicSettingPopItemView *>*showItemViews = [NSMutableArray arrayWithCapacity:4];
...
- (void)micButtonAction:(UIButton *)sender {
    ...
    if (!self.localPreviewUser) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(linkMicPopView:enableMic:)]) {
            [self.delegate linkMicPopView:self enableMic:!sender.selected];
        }
    }
}
...
- (void)cameraButtonAction:(UIButton *)sender {
    ...
    if (!self.localPreviewUser) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(linkMicPopView:enableCamera:)]) {
            [self.delegate linkMicPopView:self enableCamera:!sender.selected];
        }
    }
    if (self.currentUser.userType == PLVSocketUserTypeTeacher ||
...
- (void)cameraSwitchButtonAction:(UIButton *)sender {
    ...
    if (!self.localPreviewUser) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(linkMicPopView:cameraFront:)]) {
            [self.delegate linkMicPopView:self cameraFront:!sender.selected];
        }
    }
}
