...
/// 用户 当前是否上麦状态 (注意：仅在 [userType] 为Guests时，此值有使用意义)
...
/// 通过 传值 创建模型 (适用于本地用户)
+ (instancetype)localUserModelWithUserId:(NSString *)userId linkMicUserId:(NSString *)linkMicUserId nickname:(NSString *)nickname avatarPic:(NSString *)avatarPic userType:(PLVSocketUserType)userType;
