...
static NSString *const PLVLiveScenesSubtitleErrorDomain = @"net.polyv.subtitle.error";
...
@interface PLVLiveScenesSubtitleParser ()

@property (nonatomic, strong) NSMutableArray<PLVLiveScenesSubtitleItem *> *subtitleItems;
@property (nonatomic, strong) NSMutableArray<PLVLiveScenesSubtitleItem *> *subtitleAtTopItems;

@property (nonatomic, strong) NSDictionary<NSNumber *, PLVLiveScenesSubtitleItem *> *subtitleItemsDictionary;

@end

@implementation PLVLiveScenesSubtitleParser

#pragma mark - property

- (NSMutableArray<PLVLiveScenesSubtitleItem *> *)subtitleItems {
    if (!_subtitleItems) {
...
- (NSMutableArray<PLVLiveScenesSubtitleItem *> *)subtitleAtTopItems {
...
// abandon
- (NSDictionary<NSNumber *,PLVLiveScenesSubtitleItem *> *)subtitleItemsDictionary {
    if (!_subtitleItemsDictionary) {
        NSMutableDictionary *subtitleItemsDictionary = [NSMutableDictionary dictionary];
        for (PLVLiveScenesSubtitleItem *item in self.subtitleItems) {
            subtitleItemsDictionary[@(PLVLiveScenesSubtitleTimeGetSeconds(item.startTime))] = item;
        }
...
+ (instancetype)parserWithSubtitle:(NSString *)content error:(NSError *__autoreleasing *)error {
    PLVLiveScenesSubtitleParser *parser = [[PLVLiveScenesSubtitleParser alloc] init];
    // 字幕文本中，空行带有回车符无法解析
...
- (NSDictionary *)subtitleItemAtTime:(NSTimeInterval)time {
    PLVLiveScenesSubtitleItem * item = [self searchSubtitleWithTime:time atTop:NO];
    PLVLiveScenesSubtitleItem * itemAtTop = [self searchSubtitleWithTime:time atTop:YES];
...
- (PLVLiveScenesSubtitleItem *)searchSubtitleWithTime:(NSTimeInterval)time
                                      atTop:(BOOL)atTop{
    NSMutableArray<PLVLiveScenesSubtitleItem *> * arr = atTop ? self.subtitleAtTopItems : self.subtitleItems;
    ...
    while (low <= high) {
        ...
        PLVLiveScenesSubtitleItem *thisSub = arr[mid];
        NSTimeInterval thisStartTime = PLVLiveScenesSubtitleTimeGetSeconds(thisSub.startTime);
        
        if (thisStartTime <= time) {
            NSTimeInterval thisEndTime = PLVLiveScenesSubtitleTimeGetSeconds(thisSub.endTime);
...
#   define SCAN_STRING(str) scanString(scanner, (str))
    ...
    while (![scanner isAtEnd]) {
        ...
        PLVLiveScenesSubtitleTime start = { -1, -1, -1, -1 };
        PLVLiveScenesSubtitleTime end = { -1, -1, -1, -1 };
        ...
        if (!ok) {
            if (error) {
                ...
                NSDictionary *errorDetail = [NSDictionary dictionaryWithObjectsAndKeys:
                                             ...
                                             nil];
                *error = [NSError errorWithDomain:PLVLiveScenesSubtitleErrorDomain
                                            code:-1
        ...
        PLVLiveScenesSubtitleItem *item = [[PLVLiveScenesSubtitleItem alloc] initWithText:subText
                                                              start:start
                                                                end:end];
