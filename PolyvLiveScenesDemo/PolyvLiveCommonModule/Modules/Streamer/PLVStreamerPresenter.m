...
- (void)prepareLocalPreviewCompletion:(void (^)(BOOL, BOOL))completion {
    __weak typeof(self) weakSelf = self;
        ...
        if (self.arraySafeQueue) {
            ...
            dispatch_async(self.arraySafeQueue, ^{
                ...
                if (![userIdArray containsObject:onlineUser.linkMicUserId]) {
                    ...
                    /// 往 onlineUser 数组中添加
                    if ([onlineUser.linkMicUserId isEqualToString:weakSelf.linkMicUserId]) { // 确保本地用户总在第一位
                        [weakSelf.onlineUserMuArray insertObject:onlineUser atIndex:0];
                    } else {
