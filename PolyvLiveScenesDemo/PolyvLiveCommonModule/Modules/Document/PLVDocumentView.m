...
static NSString *kPPTUrlString = @"https://player.polyv.net/resp/ppt-h5/latest/page/pptForMobile.html";
...
- (void)jsbridge_sendSocketEventWithJson:(id)jsonObject {
    if (jsonObject) {
        ...
        if (tempDict && [tempDict isKindOfClass:[NSDictionary class]]) {
            NSString *onSliceDrawEvent = PLV_SafeStringForDictKey(tempDict, @"EVENT");
            //未开播，在接收到'sendSocketEvent' js事件时，不发送画笔'onSliceDraw'事件，其他事件正常发送
            //避免未开播的画笔事件传递到下一次直播。
            if (onSliceDrawEvent &&
                [onSliceDrawEvent isEqualToString:@"onSliceDraw"] &&
                !self.startClass) {
                return;
            }
