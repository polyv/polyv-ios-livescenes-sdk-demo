...
@interface PLVLSRemindImageMessageCell()
...
@property (nonatomic, strong) PLVLSProhibitWordTipView *prohibitWordTipView; // 严禁图提示视图
...
static NSString *otherCellId = @"PLVLSImageRemindMessageCell_Other";
static NSString *selfCellId = @"PLVLSImageRemindMessageCell_Self";

static CGFloat kCellTopMargin = 10;

@implementation PLVLSRemindImageMessageCell
...
- (void)layoutSubviews {
    ...
    NSAttributedString *attri = [PLVLSRemindImageMessageCell contentLabelAttributedStringWithProhibitWordTip:[PLVLSRemindImageMessageCell prohibitWordTip]];
    ...
    if ([self.model isProhibitMsg]) {
        ...
        imageViewSize = [PLVLSRemindImageMessageCell calculateImageViewSizeWithMessage:self.model.message];
    }
    ...
    // 自己的发言，右对齐
    if ([PLVLSRemindImageMessageCell isLoginUser:self.model.user.userId]) {
        headerImageViewX = self.cellWidth - headerWidth;
        nickNameLabelX = self.cellWidth - headerWidth - nickNameViewSize.width - bubbleXPadding;
...
- (void)updateWithModel:(PLVChatModel *)model loginUserId:(NSString *)loginUserId cellWidth:(CGFloat)cellWidth previousUserId:(NSString *)previousUserId {
    if (![PLVLSRemindImageMessageCell isModelValid:model] || cellWidth == 0) {
        self.cellWidth = 0;
        return;
    }
...
- (void)dealProhibitWordTipWithModel:(PLVChatModel *)model {
    [self.chatImageView setImage:[PLVLSUtils imageForChatroomResource:@"plvls_chatroom_cell_image_error"]];
    // 严禁词提示
...
- (void)loadImageWithModel:(PLVChatModel *)model {
    ...
    NSURL *imageURL = [PLVLSRemindImageMessageCell imageURLWithMessage:message];
...
+ (CGFloat)cellHeightWithModel:(PLVChatModel *)model loginUserId:(NSString *)loginUserId cellWidth:(CGFloat)cellWidth previousUserId:(NSString *)previousUserId {
    ...
    if ([model isProhibitMsg]) {
        ...
        CGSize imageViewSize = [PLVLSRemindImageMessageCell calculateImageViewSizeWithMessage:model.message];
...
- (UIImageView *)prohibitImageView {
    if (!_prohibitImageView) {
        ...
        _prohibitImageView.image = [PLVLSUtils imageForChatroomResource:@"plvls_chatroom_cell_icon_error"];
