...
@interface PLVLSBeautyViewModel()
...
@property (nonatomic, assign) PLVLSBeautyType beautyType; // 当前选择了什么美颜类型
...
@implementation PLVLSBeautyViewModel

#pragma mark - [ Public Method ]

+ (instancetype)sharedViewModel {
    static dispatch_once_t onceToken;
    static PLVLSBeautyViewModel *viewModel = nil;
    dispatch_once(&onceToken, ^{
        viewModel = [[PLVLSBeautyViewModel alloc] init];
    });
    return viewModel;
}
...
- (void)updateBeautyOption:(PLVBBeautyOption)beautyOption withIntensity:(CGFloat)intensity {
    plv_dict_set(self.beautyOptionDict, [NSString stringWithFormat:@"%zd", beautyOption], @(intensity));
...
- (void)updateBeautyOptionWithIntensity:(CGFloat)intensity {
    if ((self.beautyType == PLVLSBeautyTypeWhiten ||
         self.beautyType == PLVLSBeautyTypeFace) &&
...
- (void)resetBeautyOptionIntensity { // 将全部美颜强度设置为默认值并缓存
    ...
    if (self.beautyType == PLVLSBeautyTypeFilter) {
...
- (void)selectBeautyOption:(PLVBBeautyOption)option {
    ...
    if (!self.beautyIsOpen ||
        self.beautyType == PLVLSBeautyTypeFilter) {
        return;
    }
...
- (void)selectBeautyFilterOption:(PLVBFilterOption *)filterOption {
    ...
    if (self.beautyType != PLVLSBeautyTypeFilter) { // 当前没有选中滤镜模式，静默设置滤镜
        return;
    }
