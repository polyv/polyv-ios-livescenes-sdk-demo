...
- (void)refreshLinkMicButtonState{
    ...
    BOOL hiddenLinkMicButton = isLoginUser ? YES: ((self.user.waitUser || self.user.onlineUser) ? NO : YES);
...
- (void)refreshMediaControlButtonsState{
    ...
    if (self.user.onlineUser) {
        ...
        BOOL cameraButtonShow = ([PLVRoomDataManager sharedManager].roomData.channelLinkMicMediaType == PLVChannelLinkMicMediaType_Video);
        if (isLoginUser || self.user.onlineUser.userType == PLVSocketUserTypeGuest) {
