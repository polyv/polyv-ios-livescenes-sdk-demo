...
@interface PLVLCBrushColorSelectSheet()

@property (nonatomic, strong) NSArray *colorsArray; // 颜色值数组
@property (nonatomic, strong) NSArray <PLVLCBrushColorButton *> *buttonArray; // 颜色按钮数组

@end

@implementation PLVLCBrushColorSelectSheet
...
- (void)updateSelectColor:(NSString *)color localTrigger:(BOOL)localTrigger {
    ...
    [self.buttonArray enumerateObjectsUsingBlock:^(PLVLCBrushColorButton * _Nonnull button, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([self isTheSameColor:changeColor otherColor:button.color]) {
            [self colorButtonAction:button localTouch:localTrigger];
            *stop = YES;
        }
    }];
}
...
- (NSArray<PLVLCBrushColorButton *> *)buttonArray {
    if (!_buttonArray) {
