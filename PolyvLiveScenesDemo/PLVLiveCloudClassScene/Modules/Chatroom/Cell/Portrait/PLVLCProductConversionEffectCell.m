...
static CGFloat const kPLVLCConversionInnerHorizontalPadding = 10.0;
static CGFloat const kPLVLCConversionInnerVerticalPadding = 6.0;
static CGFloat const kPLVLCConversionLabelMinHeight = 19.0;
static CGFloat const kPLVLCConversionBottomPadding = 4.0;

@interface PLVLCProductConversionEffectCell ()
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        [self.bubbleView addSubview:self.chatLabel];
    }
    return self;
}
...
- (UIView *)bubbleView {
    if (!_bubbleView) {
        ...
        _bubbleView.backgroundColor = [PLVColorUtil colorFromHexString:@"#10131E" alpha:0.9];
...
- (UILabel *)chatLabel {
    if (!_chatLabel) {
        ...
        _chatLabel.lineBreakMode = NSLineBreakByTruncatingTail;
    }
    return _chatLabel;
}
...
- (void)layoutSubviews {
    ...
    if (![PLVLCProductConversionEffectCell isModelValid:self.model] || self.cellWidth <= 0) {
        return;
    }
    ...
    CGFloat labelWidth = MAX(maxBubbleWidth - kPLVLCConversionInnerHorizontalPadding * 2, 0);
    self.chatLabel.attributedText = [PLVLCProductConversionEffectCell conversionAttributedStringWithModel:self.model maxLabelWidth:labelWidth];
    ...
    CGFloat chatLabelHeight = MAX(ceil(size.height), kPLVLCConversionLabelMinHeight);
    self.chatLabel.frame = CGRectMake(kPLVLCConversionInnerHorizontalPadding, kPLVLCConversionInnerVerticalPadding, chatLabelWidth, chatLabelHeight);
    CGFloat bubbleWidth = MIN(chatLabelWidth + kPLVLCConversionInnerHorizontalPadding * 2, maxBubbleWidth);
    self.bubbleView.frame = CGRectMake(safeLeftInset, 0, bubbleWidth, chatLabelHeight + kPLVLCConversionInnerVerticalPadding * 2);
}
...
+ (BOOL)isModelValid:(PLVChatModel *)model {
    ...
    return PLV_SafeBoolForDictKey(payload, kPLVLCConversionPayloadKey);
}
...
- (void)updateWithModel:(PLVChatModel *)model
    ...
    if (![PLVLCProductConversionEffectCell isModelValid:model] || cellWidth <= 0) {
        self.model = nil;
        self.cellWidth = 0;
        self.leftInset = 0;
        self.rightInset = 0;
        self.chatLabel.attributedText = nil;
        return;
    }
    ...
    CGFloat labelWidth = MAX(maxBubbleWidth - kPLVLCConversionInnerHorizontalPadding * 2, 0);
    self.chatLabel.attributedText = [PLVLCProductConversionEffectCell conversionAttributedStringWithModel:model maxLabelWidth:labelWidth];
    [self setNeedsLayout];
}
...
+ (CGFloat)cellHeightWithModel:(PLVChatModel *)model
    ...
    CGFloat labelWidth = MAX(maxBubbleWidth - kPLVLCConversionInnerHorizontalPadding * 2, 0);
    NSAttributedString *string = [PLVLCProductConversionEffectCell conversionAttributedStringWithModel:model maxLabelWidth:labelWidth];
    if (!string) {
        return 0;
    }
    CGSize size = [string boundingRectWithSize:CGSizeMake(labelWidth, MAXFLOAT)
                                       ...
                                       context:nil].size;
    CGFloat chatLabelHeight = MAX(ceil(size.height), kPLVLCConversionLabelMinHeight);
    return chatLabelHeight + kPLVLCConversionInnerVerticalPadding * 2 + kPLVLCConversionBottomPadding;
}
