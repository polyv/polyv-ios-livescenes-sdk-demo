//
//  PLVLCLandscapeLocalSystemMessageCell.m
//  PolyvLiveScenesDemo
//
...
@interface PLVLCLandscapeLocalSystemMessageCell ()
...
@implementation PLVLCLandscapeLocalSystemMessageCell
...
- (void)layoutSubviews {
    ...
    CGFloat labelWidth = [PLVLCLandscapeLocalSystemMessageCell systemLabelWidth];
...
- (void)updateWithModel:(PLVChatModel *)model cellWidth:(CGFloat)cellWidth {
    if (cellWidth <= 0 || ![PLVLCLandscapeLocalSystemMessageCell isModelValid:model]) {
        self.cellWidth = 0;
        return;
    }
...
+ (CGFloat)cellHeightWithModel:(PLVChatModel *)model cellWidth:(CGFloat)cellWidth {
    ...
    PLVLocalSystemMessage *message = (PLVLocalSystemMessage *)model.message;
    CGFloat maxWidth = cellWidth - 24.0;
    CGFloat labelWidth = [PLVLCLandscapeLocalSystemMessageCell systemLabelWidth];
    CGFloat contentMaxWidth = maxWidth - 8.0 * 2 - labelWidth - 4.0;
    CGRect contentRect = [message.content boundingRectWithSize:CGSizeMake(contentMaxWidth, CGFLOAT_MAX)
                                                    ...
                                                    attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14.0]}
                                                       context:nil];
    return MAX(ceil(contentRect.size.height), 16.0) + 8.0;
}
