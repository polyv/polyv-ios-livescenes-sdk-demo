...
@interface PLVLCOnlineListViewCell ()
...
@implementation PLVLCOnlineListViewCell
...
- (void)updateUser:(PLVChatUser *)user {
    [self updateUser:user isLandscape:NO];
}
- (void)updateUser:(PLVChatUser *)user isLandscape:(BOOL)isLandscape {
    ...
    self.localUserLabel.textColor =  isLandscape ? [PLVColorUtil colorFromHexString:@"#000000" alpha:0.8] : [PLVColorUtil colorFromHexString:@"#FFFFFF"];
}
...
- (UILabel *)nickNameLabel {
    if (!_nickNameLabel) {
        ...
        _nickNameLabel.textColor = [PLVColorUtil colorFromHexString:@"#FFFFFF"];
    }
