...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PLVChatModel *model = [[PLVLCChatroomManager sharedManager].chatArray objectAtIndex:indexPath.row];
    if ([PLVLCLandscapeSpeakCell isModelValid:model]) {
        cellHeight = [PLVLCLandscapeSpeakCell cellHeightWithModel:model loginUserId:self.roomData.userIdForWatchUser cellWidth:self.tableView.frame.size.width];
    } else if ([PLVLCLandscapeImageCell isModelValid:model]) {
        cellHeight = [PLVLCLandscapeImageCell cellHeightWithModel:model cellWidth:self.tableView.frame.size.width];
    } else if ([PLVLCLandscapeQuoteCell isModelValid:model]) {
        cellHeight = [PLVLCLandscapeQuoteCell cellHeightWithModel:model loginUserId:self.roomData.userIdForWatchUser cellWidth:self.tableView.frame.size.width];
    } else {
        cellHeight = 0;
    }
