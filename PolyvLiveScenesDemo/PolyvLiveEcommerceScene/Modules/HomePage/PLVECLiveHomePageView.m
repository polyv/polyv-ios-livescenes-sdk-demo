...
@interface PLVECLiveHomePageView () <PLVECMoreViewDelegate, PLVPlayerSwitchViewDelegate, PLVECRewardControllerDelegate, PLVSocketLoginManagerDelegate, PLVECCommodityDelegate, PLVSocketListenerProtocol>
...
- (PLVECRewardController *)rewardCtrl {
    if (!_rewardCtrl) {
        ...
        _rewardCtrl.channel = self.roomData.channel;
...
- (PLVECCommodityView *)commodityView {
    if (!_commodityView) {
        ...
        _commodityView.channel = self.roomData.channel;
...
- (instancetype)initWithDelegate:(id<PLVECLiveHomePageViewDelegate>)delegate roomData:(PLVLiveRoomData *)roomData {
    ...
    if (self) {
        ...
        self.listenEvents = @[kPLVBSocketEvent_customMessage];
...
- (void)setupUI {
    ...
    // 初始化聊天室视图和相关配置
    self.chatroomView = [[PLVECChatroomView alloc] initWithRoomData:self.roomData];
...
- (void)socketLoginManager:(PLVSocketLoginManager *)socketLoginManager authorizationVerificationFailed:(PLVLiveRoomErrorReason)reason message:(NSString *)message {
    if ([self.delegate respondsToSelector:@selector(homePageView:authorizationVerificationFailed:message:)]) {
        [self.delegate homePageView:self authorizationVerificationFailed:reason message:message];
    }
}
...
- (void)socket:(id<PLVSocketIOProtocol>)socket didReceiveEvent:(NSString *)event jsonDict:(NSDictionary *)jsonDict {
    if ([event isEqualToString:@"customMessage"]) {
        [self customMessageEvent:jsonDict];
    }
}
...
- (void)likesEvent:(NSDictionary *)jsonDict {
    ...
    if ([userId isEqualToString:self.roomData.userIdForWatchUser]) {
        return;
    }
...
- (void)customMessageEvent:(NSDictionary *)jsonDict {
    NSString *subEvent = PLV_SafeStringForDictKey(jsonDict, @"EVENT");
...
- (void)destroy {
    ...
    [_pushView destroy];
    [[PLVECChatroomManager sharedManager] destroy];
...
- (void)likeButtonAction:(UIButton *)button {
    [self showLikeAnimation];
    [[PLVECChatroomManager sharedManager] sendLike];
}
